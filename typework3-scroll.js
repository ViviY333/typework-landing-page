/**
 * typework3.glb — 全页滚动驱动位移与旋转（独立脚本，不影响原 index.html）
 * 模型路径相对当前 HTML：./download/typework/typework3.glb
 */
// 这里使用 UMD 脚本注入（在 index-scroll3d.html 中已引入 `three.min.js` 与 `GLTFLoader.js`）
// 以便避免 ESM + importmap 兼容问题。
const GLB_URL = "./download/typework/typework3.glb";
...
function fitModelToScene(root, targetSize = 2.2) {
  ...
  // 将包围盒中心移到世界原点：worldCenter ≈ s * localCenter + position → position = -s * center
  root.position.copy(center).multiplyScalar(-s);
}

const canvas = document.getElementById("tw3-canvas");
if (!canvas) {
  console.warn("[typework3-scroll] 未找到 #tw3-canvas");
} else {
  injectStyles();
  ...
  loader.load(
    ...
    undefined,
    (err) => {
      console.error("[typework3-scroll] GLB 加载失败，请确认文件存在:", GLB_URL, err);
    }
  );

  /** 滚动进度 t∈[0,1] → 模型姿态（可按视觉再调） */
